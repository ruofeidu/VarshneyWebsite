  ...
  with open("data/%s.txt" % file_name, 'w') as f:
    ...
    if file_name == 'media':
      for m in reversed(data['media']):
        f.write(
            LINE_MEDIA % (m['publisher'], m['title'], m['url'], '(Video) ' if
                          m['video'] else '', m['month'], m['day'], m['year']))
        ...
        for m in data['students']:
          if m['category'] == c and m['visible']:
            if count and count % 5 == 0:
              ...
              f.write(NEW_ROW)
            f.write(
                LINE_STUDENTS % (m['url'], m['photo'], m['name'] + "'s photo",
                                 m['url'], m['name']))
            count += 1
        f.write(ROW_END)
...
print("Done! %d papers rendered. Time used %.2fs" % (NUM_PAPERS,
